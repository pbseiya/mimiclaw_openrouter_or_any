...
このフォークには、MimiClawをより使いやすく、より柔軟にするためのいくつかの拡張機能が含まれています：
- **OpenRouter サポート**: OpenRouterのAPIエンドポイントで動作するように完全にパッチされています。
- **環境変数の管理**: クレデンシャルを `.env` ファイルで安全に管理できます。
- **自動化スクリプト**: `./build_and_flash.sh` を使用して、シークレットの同期、ビルド、フラッシュを1つのコマンドで実行できます。
- **カスタムドキュメント**: 詳細なローカルガイドは [README_INSTALL.md](README_INSTALL.md) にあります。
